...
function toolStatusName(id) { return id === "rect" ? "rec" : id; }
function setTool(id) { tool = id; setupTools(); setStatus(`${toolStatusName(id)} selected.`); }
...
function syncControls() { canvasSizeSelect.value = String(state.size); frameDurationInput.value = frame().duration; layerOpacityInput.value = Math.round(layer().opacity * 100); syncColorInputs(color); }
...
function resizeProject(size, keepArtwork = true) {
  pushHistory();
  if (keepArtwork) {
    state.frames.forEach(f => {
      f.layers.forEach(l => {
        l.pixels = resamplePixelsToProject(l.pixels, size);
      });
    });
    state.size = size;
    syncControls();
    renderAll();
    setStatus(`Resized canvas to ${size} by ${size}.`);
    return;
  }
...
canvasSizeSelect.onchange = e => resizeProject(Number(e.target.value), true);
...
$("#new-project-btn").onclick = () => resizeProject(state.size, false); $("#save-project-btn").onclick = saveProject; $("#open-project-btn").onclick = openProject; $("#export-png-btn").onclick = exportPNG; $("#export-sheet-btn").onclick = exportSheet; $("#export-gif-btn").onclick = exportGIF;
...
window.addEventListener("keydown", e => { if(e.key === "Escape" && !pixelizerOverlay.hidden) { closePixelizerModal(); return; } if(e.key === "Escape" && !privacyOverlay.hidden) { closePrivacyModal(); return; } const mod=e.ctrlKey||e.metaKey; if(mod&&e.key.toLowerCase()==="s"){e.preventDefault();saveProject();} if(mod&&e.key.toLowerCase()==="o"){e.preventDefault();openProject();} if(mod&&e.key.toLowerCase()==="z"){e.preventDefault(); e.shiftKey ? $("#redo-btn").click() : $("#undo-btn").click();} if(mod&&e.key.toLowerCase()==="y"){e.preventDefault();$("#redo-btn").click();} const map={b:"pencil",e:"eraser",g:"fill",i:"eyedropper",l:"line",r:"rect",o:"ellipse"}; if(!mod && map[e.key.toLowerCase()]) setTool(map[e.key.toLowerCase()]); });
loadDockLayout();
setupDockRearrange();
