...
// Canvas draw
function renderAll() { drawCanvas(); renderFrames(); renderLayers(); renderPalette(); renderStoryboard(); if (printMode) schedulePrintPreview(); saveLocal(); }
      ...
      for (let y = 0; y < state.size; y += step) {
        for (let x = 0; x < state.size; x += step) {
          ...
          source.set(`${Math.floor(x / step)},${Math.floor(y / step)}`, {
            ...
            h: 1,
            color: colorValue,
            depth: mode === "brightness" || mode === "brightness-reverse" ? voxelBrightnessDepth(colorValue, baseDepth, mode === "brightness-reverse") : baseDepth
          });
        }
      }
    });
...
function voxelTransform(x, y, z, size, depth, angleX, angleY) {
  let px = x - size / 2;
  let py = y - size / 2;
  let pz = z - depth / 2;
...
function voxelCorner(x, y, z, size, depth, angleX, angleY, scale, originX, originY) {
  const point = voxelTransform(x, y, z, size, depth, angleX, angleY);
...
function voxelPrismFaces(cube, scale, originX, originY, angleX, angleY) {
  ...
  const z0 = cube.z || 0;
  const z1 = z0 + depth;
  const p000 = voxelCorner(x, y, z0, size, depth, angleX, angleY, scale, originX, originY);
  const p100 = voxelCorner(x + w, y, z0, size, depth, angleX, angleY, scale, originX, originY);
  const p010 = voxelCorner(x, y + h, z0, size, depth, angleX, angleY, scale, originX, originY);
  const p110 = voxelCorner(x + w, y + h, z0, size, depth, angleX, angleY, scale, originX, originY);
  const p001 = voxelCorner(x, y, z1, size, depth, angleX, angleY, scale, originX, originY);
  const p101 = voxelCorner(x + w, y, z1, size, depth, angleX, angleY, scale, originX, originY);
  const p011 = voxelCorner(x, y + h, z1, size, depth, angleX, angleY, scale, originX, originY);
  const p111 = voxelCorner(x + w, y + h, z1, size, depth, angleX, angleY, scale, originX, originY);
  return [
...
function drawVoxelFloor(scale, originX, originY, angleX, angleY) {
  const size = voxelPreviewSize();
  const depth = Number(voxelDepthInput.value);
  const y = size + 1;
  const a = voxelCorner(-1, y, -1, size, depth, angleX, angleY, scale, originX, originY);
  const b = voxelCorner(size + 1, y, -1, size, depth, angleX, angleY, scale, originX, originY);
  const c = voxelCorner(size + 1, y, depth + 1, size, depth, angleX, angleY, scale, originX, originY);
  const d = voxelCorner(-1, y, depth + 1, size, depth, angleX, angleY, scale, originX, originY);
...
function drawVoxelPreview() {
  ...
  drawVoxelBackdrop();
  if (voxelFloorInput.checked) drawVoxelFloor(scale, originX, originY, angleX, angleY);
  if (voxelPreviewStep() > 1) setStatus(`Voxel preview simplified for ${state.size} by ${state.size}.`);
  const faces = [];
  voxelPixels().forEach(cube => faces.push(...voxelPrismFaces(cube, scale, originX, originY, angleX, angleY)));
...
function printSettings() {
  ...
  return { dpi, artWidth, bleed, safe, stripWidth, stripHeight, stickerCols, stickerRows, stickerGap, memoHeight, mode: printRepeatSelect.value };
}
...
function makePrintCanvas(settings) {
...
function drawPrintPreview() {
  ...
  const output = makePrintCanvas(settings);
...
[voxelDepthInput, voxelScaleInput, voxelRotationXInput, voxelRotationYInput].forEach(input => input.oninput = () => { if (input === voxelDepthInput) { voxelPixelCache = null; voxelNeighborCache = null; } scheduleVoxelPreview(); });
