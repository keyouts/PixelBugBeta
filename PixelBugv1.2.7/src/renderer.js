...
function floodFill(x, y, target, replacement, pixels) {
  ...
  const boundary = enclosedBoundaryColor(x, y, target, pixels);
  if (boundary) return enclosedFill(x, y, boundary, replacement, pixels);
  const stack = [[x, y]];
  while (stack.length) {
    const [cx, cy] = stack.pop();
    if (cx < 0 || cy < 0 || cx >= state.size || cy >= state.size || pixels[cy][cx] !== target) continue;
    pixels[cy][cx] = replacement;
    stack.push([cx + 1, cy], [cx - 1, cy], [cx, cy + 1], [cx, cy - 1]);
  }
}

function enclosedBoundaryColor(x, y, target, pixels) {
  const seen = Array.from({ length: state.size }, () => Array(state.size).fill(false));
  const stack = [[x, y]];
  const touchesEdge = { value: false };
  const counts = new Map();
  while (stack.length) {
    const [cx, cy] = stack.pop();
    if (cx < 0 || cy < 0 || cx >= state.size || cy >= state.size || seen[cy][cx] || pixels[cy][cx] !== target) continue;
    seen[cy][cx] = true;
    if (cx === 0 || cy === 0 || cx === state.size - 1 || cy === state.size - 1) touchesEdge.value = true;
    [[1,0],[-1,0],[0,1],[0,-1]].forEach(([dx, dy]) => {
      const nx = cx + dx;
      const ny = cy + dy;
      if (nx < 0 || ny < 0 || nx >= state.size || ny >= state.size) return;
      const next = pixels[ny][nx];
      if (next === target && !seen[ny][nx]) stack.push([nx, ny]);
      else if (next && next !== target) counts.set(next, (counts.get(next) || 0) + 1);
    });
  }
  if (touchesEdge.value || !counts.size) return null;
  return Array.from(counts.entries()).sort((a, b) => b[1] - a[1])[0][0];
}

function enclosedFill(x, y, boundary, replacement, pixels) {
  const stack = [[x, y]];
  const seen = Array.from({ length: state.size }, () => Array(state.size).fill(false));
  while (stack.length) {
    const [cx, cy] = stack.pop();
    if (cx < 0 || cy < 0 || cx >= state.size || cy >= state.size || seen[cy][cx] || pixels[cy][cx] === boundary) continue;
    seen[cy][cx] = true;
    pixels[cy][cx] = replacement;
    stack.push([cx + 1, cy], [cx - 1, cy], [cx, cy + 1], [cx, cy - 1]);
  }
