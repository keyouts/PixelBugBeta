...
function setTool(id) { tool = id; setupTools(); setStatus(`${id} selected.`); }
...
function resizeProject(size) { pushHistory(); state = freshProject(size); undoStack = []; redoStack = []; syncControls(); renderAll(); setStatus(`New ${size} by ${size} project.`); }
...
$("#new-project-btn").onclick = () => resizeProject(state.size); $("#save-project-btn").onclick = saveProject; $("#open-project-btn").onclick = openProject; $("#export-png-btn").onclick = exportPNG; $("#export-sheet-btn").onclick = exportSheet; $("#export-gif-btn").onclick = exportGIF;
